...
4;boss.scaleY=4;boss.vx=Math.floor(11*Math.random())+-5;boss.vy=Math.floor(11*Math.random())+-5;boss.dead=!1;gameScene.addChild(boss);if(level1||level2){var a=g.rectangle(128,16,"black"),b=g.rectangle(128,16,"yellowGreen");bossHealth=g.group(a,b);bossHealth.inner=b;bossHealth.x=g.canvas.width-375;bossHealth.y=20}else{a=g.rectangle(128,16,"black");b=g.rectangle(128,16,"yellowGreen");var d=g.rectangle(128,16,"yellow");bossHealth=g.group(a,b,d);bossHealth.inner=b;bossHealth.armor=d;bossHealth.x=g.canvas.width-
350;bossHealth.y=30}gameScene.addChild(bossHealth);if(level2||level4)healthPotion=g.sprite("./assets/health.png"),healthPotion.x=g.canvas.width-healthPotion.width-60,healthPotion.y=g.canvas.height-188-healthPotion.height-30,healthPotion.scaleX=4,healthPotion.scaleY=4,gameScene.addChild(healthPotion);(level4||level5)&&walls.forEach(function(a){gameScene.removeChild(a)});bulletTimer=0;bossBullets=[];player.x=g.canvas.width/2-player.halfWidth;player.y=g.canvas.height-188-player.height-45;healed=!1;g.state=
bossPlay}
function bossPlay(){var a=!1;g.move(player);g.move(boss);g.contain(player,{x:40,y:40,width:475,height:500});var b=g.contain(boss,{x:10,y:23,width:498,height:492});if("left"===b||"right"===b||"top"===b||"bottom"===b){var d=b=0;b=Math.floor(11*Math.random())+-5;d=Math.floor(11*Math.random())+-5;boss.vx=b;boss.vy=d}if(hasSword||g.hitTestRectangle(player,sword))hasSword=!0,sword.visible=!1,1==lastDirection?(angleToShoot=3.14,swordswing.x=player.x-32,swordswing.y=player.y):2==lastDirection?(angleToShoot=0,
swordswing.x=player.x+32,swordswing.y=player.y):3==lastDirection?(angleToShoot=4.71,swordswing.y=player.y-32,swordswing.x=player.x):4==lastDirection?(angleToShoot=1.57,swordswing.y=player.y+32,swordswing.x=player.x):(angleToShoot=4.71,swordswing.x=player.x-16,swordswing.y=player.y-16);g.key.space.press=function(){1==hasSword&&(swordswing.visible=!0);g.hitTestCircleRectangle(swordswing,boss)&&(level3?0==bossArmor&&(bossHealth.inner.width-=5):bossHealth.inner.width-=5)};g.key.space.release=function(){swordswing.visible=
